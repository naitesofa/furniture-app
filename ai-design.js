...
document.addEventListener('DOMContentLoaded', function() {
  ...
  let currentRoom = '风格';
  // 当前设计模式
  let currentDesignType = 'rough'; // 默认为毛坯房设计
  
  // 设计类型面板处理函数
  function openDesignPanel(type) {
    currentDesignType = type;
    
    // 根据不同设计类型调整UI和功能
    switch(type) {
      case 'rough': // 毛坯房设计
        composer.placeholder = "描述您想要的毛坯房设计...";
        addParamPill('type', '毛坯房设计');
        // 设置默认参数和UI状态
        showAvailableOptions('rough');
        break;
        
      case 'new': // 新房软装设计
        composer.placeholder = "描述您的软装风格需求...";
        addParamPill('type', '新房软装设计');
        // 设置默认参数和UI状态
        showAvailableOptions('new');
        break;
        
      case 'old': // 旧房改造
        composer.placeholder = "描述您想要改造的旧房空间...";
        addParamPill('type', '旧屋改造');
        // 设置默认参数和UI状态
        showAvailableOptions('old');
        break;
        
      case 'text2img': // 文生图模式
        composer.placeholder = "详细描述您想要生成的图像...";
        addParamPill('type', '文生图模式');
        // 设置文生图专用参数和UI状态
        showAvailableOptions('text2img');
        break;
        
      default:
        composer.placeholder = "描述您想要的设计...";
        break;
    }
    ...
    console.log(`设计模式已切换到: ${type}`);
  }
  
  // 显示特定设计类型可用的选项
  function showAvailableOptions(type) {
    // 这里可以根据不同设计类型显示不同的选项
    // 例如文生图模式可能需要不同的比例选项或风格选项
    if (type === 'text2img') {
      // 为文生图模式设置特殊选项
      document.querySelector('[data-room="比例"]').textContent = '16:9'; // 默认比例改为16:9
      // 这里可以添加其他文生图特有的设置
    } else {
      // 恢复默认选项
      document.querySelector('[data-room="比例"]').textContent = '3:4';
    }
  }
  
  // 将openDesignPanel函数暴露给全局
  window.openDesignPanel = openDesignPanel;
  ...
  function formatTextWithKeywords(text) {
    ...
    // 设计类型关键词
    const designTypes = ['毛坯图', '毛坯房设计', '旧房', '旧屋改造', '新房软装', '文生图'];
...
if (creativeTextarea) {
  ...
  if (generateBtn) {
    generateBtn.addEventListener('click', function() {
      ...
      const currentDesignType = window.currentDesignType || 'rough';
      
      // 检查：当选择了"毛坯房设计"模式但没有上传图片时，显示提示
      if (currentDesignType === 'rough' && !hasReference) {
        // 创建移动端友好的提示框
        showMobileToast('请上传毛坯房照片', true);
        
        // 高亮上传图片按钮以引导用户
        const uploadImageBtn = document.getElementById('uploadImageBtn');
        if (uploadImageBtn) {
          // 添加动画效果突出显示上传按钮
          uploadImageBtn.classList.add('upload-attention');
          setTimeout(() => {
            uploadImageBtn.classList.remove('upload-attention');
          }, 2000);
        }
      ...
      const generationData = { 
        ...
        selectedRoom,
        selectedRatio,
        designType: currentDesignType, // 添加设计类型
        // 提取文本中的风格和户型信息
  ...
  function showMobileToast(message, isWarning = false) {
    ...
    toast.className = `mobile-toast ${isWarning ? 'warning' : ''}`;
    ...
    Object.assign(toast.style, {
      ...
      backgroundColor: isWarning ? 'rgba(220, 53, 69, 0.9)' : 'rgba(0, 0, 0, 0.8)',
      color: 'white',
      padding: '12px 20px',
      borderRadius: '24px',
      fontSize: '15px',
